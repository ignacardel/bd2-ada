...
CONSTRAINT PK_PREMIO_MIEMBRO PRIMARY KEY(PM_PREMIO,PM_MIEMBRO,PM_FECHA,PM_TIPO_VOTO),
...
PELI_CENSURA VARCHAR(5) NOT NULL,
...
PERS_FECHA_MUERTE date NULL,
PERS_SEXO VARCHAR2(2) NULL,
CONSTRAINT PK_PERSONA PRIMARY KEY(PERS_ID),
CONSTRAINT CH_PERS_SEXO CHECK (PERS_SEXO IN ('M','F'))
);
...
CONSTRAINT CH_FAMI_TIPO CHECK (FAMI_TIPO IN('HIJO','HIJA','PADRE','MADRE','SOBRINO','SOBRINA','TIO','TIA','PRIMO','PRIMA','HERMANO','HERMANA','ABUELO','ABUELA','NIETO','NIETA','ESPOSA','ESPOSO'))
);
...
CONSTRAINT CH_LUGAR_TIPO CHECK(LUGA_TIPO IN('CONTINENTE','PAIS','ESTADO','CIUDAD'))
);
